			...
			if (index) {
				...
				while (UserFlag) {
					...
					case 1: {
						...
						cout << "Enter Destination\n";
						cin >> Destination;
						if (!ShowDateVehicle(Origin, Destination)) {
							cout << "There Is No Trip Press Enter To Exit\n";
							cin.ignore();
							cin.ignore();
							break;
						}
						...
						cout<<"Enter Vehicle\n";
						cin>>Vehicle;*/
						int tripindex = TripIndexer(help, Destination, Origin);
						Date = TripInf[tripindex].Date;
						Vehicle = TripInf[tripindex].Vehicle;
						if (ShowTrip(Origin, Destination, Date, Vehicle)) {
							...
							if (SeatExist(Seat, Origin, Destination, Date, Vehicle, DriverInf[DriverId(TripInf[index].Id)].Firstname, TripInf[index].Time)) {
								//cout<<"Enter Bank Id\n";
								//cin>>Bid;
								int indexer = IsCustomerExist(username, Password);
								indexer--;
								Bid = CustomerList[indexer].BId;
								cout << endl << Bid << endl;
								cout << "Enter Bank Password";
								cin >> Bpassword;
								if (IsCorrent(Bid, Bpassword)) {
